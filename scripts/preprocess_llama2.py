...
TVSUM_VIDEO_DIR = "/DATA/CV/Data/datasets/ydata-tvsum50-v1_1/video/*.mp4"
TVSUM_MAT_PATH = "/DATA/CV/Data/datasets/ydata-tvsum50-v1_1/matlab/ydata-tvsum50.mat"
SAVE_TVSUM = "/DATA/CV/processed_data_llama2/tvsum" # Changed folder name to avoid overwriting

SUMME_VIDEO_DIR = "/DATA/CV/Data/datasets/SumMe/videos/*.mp4"
SUMME_GT_DIR = "/DATA/CV/Data/datasets/SumMe" 
SAVE_SUMME = "/DATA/CV/processed_data_llama2/summe" # Changed folder name
...
class VideoProcessor:
    ...
    # -------------------------------------------------------
    def get_multimodal_prompts(self, video_path, use_audio=True, sample_fps=1):
    ...
    # -------------------------------------------------------
    # STEP 2: GENERATE EMBEDDINGS (Llama 2 Version)
    # -------------------------------------------------------
    def get_llama_embeddings(self, prompts, window_size=5):
        print(f"   [2/2] Embedding with Llama 2 (Window {window_size})...")
        ...
        # --- CHANGE 2: MANUAL STRING FORMATTING FOR LLAMA 2 ---
        # Llama 2 uses [INST] <<SYS>> ... <</SYS>> ... [/INST] structure
        
        sys_prompt = "You are a professional video editor. Analyze the 'TARGET FRAME' context to determine its importance."
        
        # Hardcoding the few-shot examples into the text prompt because
        # apply_chat_template behavior varies for Llama 2 across transformer versions
        few_shot_text = (
            "Example 1:\n"
            ...
            "Analysis: Low importance. Repetitive visual content with no significant action.\n\n"
            "Example 2:\n"
            ...
            "Analysis: High importance. Climax of the event with distinct visual action and audio intensity.\n"
        )
        ...
        for i in tqdm(range(0, len(context_inputs), batch_size), desc="     Embedding", leave=False):
            ...
            formatted_batch = []
            for txt in batch:
                # Construct Llama 2 Prompt manually
                prompt = f"[INST] <<SYS>>\n{sys_prompt}\n<</SYS>>\n\nHere are examples:\n{few_shot_text}\n\nNow analyze this context:\n{txt} [/INST]"
                formatted_batch.append(prompt)
                
            inputs = tokenizer(formatted_batch, return_tensors="pt", padding=True, truncation=True, max_length=2048).to(DEVICE)
